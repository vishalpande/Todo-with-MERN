...
class EditTodo extends Component{
    ...
    onSubmit(e) {
        ...
        console.log(obj);
        axios.put('http://localhost:4000/todos/update/' + this.props.params.id, obj).then(res => console.log(res.data));

        this.props.navigate('/');
    }

    componentDidMount() {
        axios.get('http://localhost:4000/todos/' + this.props.params.id).then(response => {
            this.setState({todo_description: response.data[0].todo_description, todo_responsible: response.data[0].todo_responsible, todo_priority: response.data[0].todo_priority, todo_complete: response.data[0].todo_complete})
                ...
                <div className="form-group">
                    ...
                    <div className="form-check form-check-inline">
                        <input className="form-check-input" type="radio" name="priorityOptions" id="priorityMedium" value="Medium"
                            checked={
                                this.state.todo_priority === 'Medium'
                            }
                            onChange={
                                this.onChangeTodoPriority
                            }/>
